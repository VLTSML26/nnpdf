...
def plot_chi2dist_experiments(total_chi2_data, experiments_chi2_stats, pdf):
    ...
    fig, ax = _chi2_distribution_plots(
        total_chi2_data, experiments_chi2_stats, pdf, "hist"
    )
...
def kde_chi2dist_experiments(total_chi2_data, experiments_chi2_stats, pdf):
    ...
    fig, ax = _chi2_distribution_plots(
        total_chi2_data, experiments_chi2_stats, pdf, "kde"
    )
...
def _chi2_distribution_plots(chi2_data, stats, pdf, plot_type):
    ...
    if not isinstance(alldata, MCStats):
        ...
        log.warning(
            "Chi² distribution plots have a " "different meaning for non MC sets."
        )
        label += " (%s!)" % pdf.error_type
    label += "\n" + "\n".join(
        str(chi2_stat_labels[k]) + (" %.2f" % v) for (k, v) in stats.items()
    )
...
def _check_same_group_data_name(dataspecs_groups):
    ...
    for j, x in enumerate(lst[1:]):
        if len(x) != len(lst[0]):
            raise CheckError(
                "All dataspecs should have the same number " "of groups of data"
            )
                ...
                raise CheckError(
                    "\n".join(
                        [
                            ...
                            fr"dataspec {j+1}, " fr"group {i+1}: {group.name}",
                            fr"dataspec 1, group {i+1}: " fr"{lst[0][i].name}",
                        ]
                    )
                )
...
def _plot_fancy_impl(
    ...
    for samefig_vals, fig_data in figby:
        ...
        ax.set_title(
            "%s %s"
            % (info.dataset_label, info.group_label(samefig_vals, info.figure_by))
        )
        ...
        for (sameline_vals, line_data) in lineby:
            ...
            if normalize_to is not None:
                ...
                annotate_point = (
                    x[-1],
                    line_data[("cv", next_after_normalize)].values[-1],
                )
            else:
...
@figuregen
def plot_fancy(
    one_or_more_results, commondata, cuts, normalize_to: (int, str, type(None)) = None
):
    """
...
@figure
def plot_fits_chi2_spider(
    fits, fits_groups_chi2, fits_groups_data, processed_metadata_group
):
...
@figure
def plot_groups_data_chi2_spider(
    groups_data, groups_chi2, processed_metadata_group, pdf
):
...
@figure
def plot_groups_data_phi_spider(
    groups_data, groups_data_phi, processed_metadata_group, pdf
):
...
plot_experiments_chi2 = collect(
    "plot_groups_data_chi2", ("group_dataset_inputs_by_experiment",)
)
...
def plot_dataspecs_groups_chi2(dataspecs_groups_chi2_table, processed_metadata_group):
    ...
    return plot_fits_groups_data_chi2(
        dataspecs_groups_chi2_table, processed_metadata_group
    )
...
@figure
def plot_positivity(
    pdfs, positivity_predictions_for_pdfs, posdataset, pos_use_kin=False
):
...
def _check_highlights(data_input, highlight_datasets):
    if highlight_datasets:
        ...
        if diff:
            raise CheckError(
                f"The following highlight elements are " "not dataset names: {diff}"
            )
...
def plot_xq2(
    ...
    for (
        ...
        masked,
        group,
    ) in dataset_inputs_by_groups_xq2map:
